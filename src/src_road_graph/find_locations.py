...
def result_to_location(result: dict) -> tuple[CustomerNode, str]:
    rating = result.get("rating", random.randrange(0, 5))
    return (CustomerNode(result['geometry']['location']['lat'], result['geometry']['location']['lng'], result['name'], rating, "C0"), result['name'])
